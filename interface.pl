...
menu:- nl,write('Bem-vindo ao sistema de criacao de itenerarios!'), nl,nl,
               write('Qual a sua preferencia no percurso que quer fazer?'),nl,nl,
               write('1. Percurso com menor custo.'),nl,
               write('2. Percurso com menor distancia.'),nl,
               write('3. Percurso mais vantajoso.'),nl,
               read(C),nl,
...
questao3(A,B,C) :- ((1==C),nl, nl, write('Percurso com menor custo de '), write(B), write(' ate '), write(A), nl, nl, resposta2(A,B);
					...
					(3==C),nl, nl, write('Percurso mais vantajoso de '), write(B), write(' ate '), write(A),nl, nl, resposta4(A,B)
).
...
resposta4(A,B):- caminhoMaisVantajoso(A,B,C),write(C),nl, finalizar().
